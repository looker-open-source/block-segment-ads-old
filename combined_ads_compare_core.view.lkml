    ...
    sql: with
      fb_perf as (
        select  a.id as ad_id,
          ...
          from  @{FACEBOOK_ADS_SCHEMA_NAME}.ads a
          join  @{FACEBOOK_ADS_SCHEMA_NAME}.insights i
            on  a.id = i.ad_id
          join  @{FACEBOOK_ADS_SCHEMA_NAME}.campaigns c
            on  a.campaign_id = c.id
          join  @{FACEBOOK_ADS_SCHEMA_NAME}.ad_sets ad
            on  a.adset_id = ad.id
      group by  1,2,3,4
      ),
      google_perf as (
        select  a.id as ad_id,
          ...
          from  @{GOOGLE_ADWORDS_SCHEMA_NAME}.ads a
          join  @{GOOGLE_ADWORDS_SCHEMA_NAME}.ad_performance_reports apr
            on  a.id = apr.ad_id
          join  @{GOOGLE_ADWORDS_SCHEMA_NAME}.ad_groups g
            on  a.ad_group_id = g.id
          join  @{GOOGLE_ADWORDS_SCHEMA_NAME}.campaigns c
